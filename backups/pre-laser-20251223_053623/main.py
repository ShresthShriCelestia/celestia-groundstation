...
async def _poll_laser_telemetry():
    """
    Background task to periodically poll laser status and update shared state.
    Runs at 1 Hz to avoid overwhelming the laser controller.
    """
    ...
    while True:
        try:
            # Get laser decoder (or skip if not available)
            try:
                decoder = get_laser_decoder()
                telemetry = decoder.get_laser_telemetry()
                await state.update_laser_telemetry(telemetry)
            except Exception as e:
                # Silently fail if laser not available - this is expected during mock testing
                pass
...
async def lifespan(app: FastAPI):
    ...
    # PX4 connection is now optional - no auto-connect on startup
    # To enable PX4: POST /px4/connect after server starts
    print("PX4 auto-connect disabled - use /px4/connect endpoint if needed")

    # Start background laser telemetry polling
...
async def stop_ramp():
    ...
    try:
        # If PX4 is connected, land the drone FIRST
        if supervisor.px4 and supervisor.px4._drone:
            try:
                ...
                print(f"[API] Landing failed (PX4 may not be connected): {e}")
                # Continue to stop processes anyway
        else:
            print("[API] PX4 not connected - skipping drone landing")

        # Then stop all processes
        await supervisor.stop_all()

        return {"status": "stopped", "message": "Ramp stopped"}
    except Exception as e:
            ...
            try:
                ...
                message = {
                    ...
                    "permit": {
                        ...
                        "bypass_enabled": settings.BYPASS_PHOTON_HANDSHAKE,  # Safety bypass for testing
                    },
                    "battery": {
                    ...
                    "laser": {
                        ...
                        "output_power_w": telemetry.get("laser_output_power_w", 0.0),
                        "temperature_c": telemetry.get("laser_temperature_c", 0.0),
                        "setpoint_pct": telemetry.get("laser_setpoint_pct", 0.0),
                        "emission_on": telemetry.get("laser_emission_on", False),
                        "power_supply_on": telemetry.get("laser_power_supply_on", False),
                        "alarm_critical": telemetry.get("laser_alarm_critical", False),
                        "alarm_overheat": telemetry.get("laser_alarm_overheat", False),
